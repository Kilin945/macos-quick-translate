...
# 429 cooldown: the unofficial gtx endpoint rate-limits by the minute, so retrying in 1s is
# pointless and every blocked run wastes ~3s before falling back. After a 429, a marker file
# makes subsequent runs (each hotkey press is a fresh process) go straight to the offline
# translator; once the cooldown expires the API is tried again, re-arming on another 429.
COOLDOWN_FILE = '/tmp/translate_429_until'
...
def start_google_cooldown():
    try:
        with open(COOLDOWN_FILE, 'w') as f:
            f.write(str(time.time() + COOLDOWN_SECONDS))
    except Exception:
        pass
...
def translate(text):
    ...
    for attempt in range(2):
        try:
            ...
            if e.code == 429:
                # rate-limited: a 1s retry never clears it — arm the cooldown and bail out
                start_google_cooldown()
                log(f'translate got HTTP 429, cooling down google for {COOLDOWN_SECONDS // 60}min')
                raise
            last_err = e
...
result = None
if google_cooldown_active():
    # recently rate-limited — skip the doomed API call and its wasted seconds
    log('google in 429 cooldown, going straight to offline')
    result = translate_offline(text)
    if result:
        log(f'offline (cooldown) OK: {_preview(result)}')
    # offline unavailable (non-en input, missing helper): fall through and try google anyway
