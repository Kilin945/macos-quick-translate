...
def _run_dialog_page(chunk, title, buttons, default_button):
    ...
    return subprocess.run(['osascript', '-e', script, '--', chunk, title], capture_output=True, text=True)
...
def show_dialog(text):
    ...
    while 0 <= i < total:
        ...
        if total == 1:
            _run_dialog_page(chunk, title, ['Close'], 'Close')
            break
        elif i == 0:
            r = _run_dialog_page(chunk, title, ['Close', 'Next'], 'Next')
            if 'Next' in r.stdout: i += 1
            else: break
        elif i == total - 1:
            r = _run_dialog_page(chunk, title, ['Previous', 'Close'], 'Close')
            if 'Previous' in r.stdout: i -= 1
            else: break
        else:
            r = _run_dialog_page(chunk, title, ['Previous', 'Close', 'Next'], 'Next')
            if 'Next' in r.stdout: i += 1
            elif 'Previous' in r.stdout: i -= 1
            else: break
